#!/usr/bin/env python3
"""
Replace the generatePage function in scripts/generate-static-pages.mjs
with the updated version that uses the full homepage header/nav/footer.
"""
import re

SCRIPT = '/home/ubuntu/cfwforever/scripts/generate-static-pages.mjs'

with open(SCRIPT, encoding='utf-8') as f:
    content = f.read()

# The new generatePage function
NEW_FUNC = r'''function generatePage(page) {
...
# Find the exact boundaries of the old generatePage function
start_marker = 'function generatePage(page) {'
...
if start_idx == -1:
    print("ERROR: Could not find generatePage function start")
    exit(1)
if end_idx == -1:
    print("ERROR: Could not find end marker")
    exit(1)

print(f"Found generatePage at lines {content[:start_idx].count(chr(10))+1} to {content[:end_idx].count(chr(10))+1}")
...
print("Successfully patched generate-static-pages.mjs")
print(f"Old function size: {end_idx - start_idx} chars")
print(f"New function size: {len(NEW_FUNC)} chars")
