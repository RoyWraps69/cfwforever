exports.handler = async (event) => {
  ...
  if (!q.code && RT) {
    ...
    const [tot,qr,pr,dr] = await Promise.all([
      ...
      call({startDate:"2026-01-10",endDate:"2026-04-10",dimensions:["page"],rowLimit:50}),
      call({startDate:"2025-10-01",endDate:"2026-01-09",dimensions:[]}),
    ]);
    const t=tot.rows?.[0]||{}, prev=dr.rows?.[0]||{};
    const clickDelta = t.clicks - prev.clicks;
    const ctrDelta = ((t.ctr||0)-(prev.ctr||0))*100;
    const zeroClick = (qr.rows||[]).filter(r=>r.clicks===0&&r.impressions>=50).sort((a,b)=>b.impressions-a.impressions);
    const highPos = (qr.rows||[]).filter(r=>r.position<=5&&r.impressions>=50).sort((a,b)=>b.impressions-a.impressions);
    ...
    let html = `
    <p style="color:#888;margin-bottom:24px">Jan 10 – Apr 10, 2026 vs Oct 1 – Jan 9, 2026</p>
    ...
    ${tbl(zeroClick.slice(0,20),[
      {h:'Query',fn:r=>r.keys[0],gold:true},{h:'Impr',r:true,fn:r=>r.impressions.toLocaleString()},{h:'Pos',r:true,fn:r=>r.position.toFixed(1)}
    ])}

    <h3 style="color:#FFD700;margin-bottom:12px">🟡 Low CTR — Position Good, Title/Meta Failing</h3>
    ${tbl(lowCTR.slice(0,15),[
      {h:'Query',fn:r=>r.keys[0],gold:true},{h:'Impr',r:true,fn:r=>r.impressions.toLocaleString()},{h:'CTR',r:true,fn:r=>(r.ctr*100).toFixed(2)+'%'},{h:'Pos',r:true,fn:r=>r.position.toFixed(1)}
    ])}

    <h3 style="color:#FFD700;margin-bottom:12px">🟢 Top 5 Position Wins — Need CTR Boost</h3>
    ${tbl(highPos.slice(0,15),[
      {h:'Query',fn:r=>r.keys[0],gold:true},{h:'Clicks',r:true,fn:r=>r.clicks},{h:'Impr',r:true,fn:r=>r.impressions.toLocaleString()},{h:'CTR',r:true,fn:r=>(r.ctr*100).toFixed(1)+'%'},{h:'Pos',r:true,fn:r=>r.position.toFixed(1)}
    ])}

    <h3 style="color:#FFD700;margin-bottom:12px">📄 Top Pages by Clicks</h3>
    ${tbl((pr.rows||[]).slice(0,25),[
      {h:'Page',fn:r=>r.keys[0].replace('https://chicagofleetwraps.com',''),gold:true},{h:'Clicks',r:true,fn:r=>r.clicks},{h:'Impr',r:true,fn:r=>r.impressions.toLocaleString()},{h:'CTR',r:true,fn:r=>(r.ctr*100).toFixed(1)+'%'},{h:'Pos',r:true,fn:r=>r.position.toFixed(1)}
    ])}

    <h3 style="color:#FFD700;margin-bottom:12px">📊 All Queries (Top 200)</h3>
    ${tbl((qr.rows||[]),[
      {h:'Query',fn:r=>r.keys[0],gold:true},{h:'Clicks',r:true,fn:r=>r.clicks},{h:'Impr',r:true,fn:r=>r.impressions.toLocaleString()},{h:'CTR',r:true,fn:r=>(r.ctr*100).toFixed(1)+'%'},{h:'Pos',r:true,fn:r=>r.position.toFixed(1)}
    ])}`;
    return pg("🔍 GSC Audit — chicagofleetwraps.com",html);
  }
  if (!q.code) return pg("Ready","<p>No refresh token in env.</p>");
